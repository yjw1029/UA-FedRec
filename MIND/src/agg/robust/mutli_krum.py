...
class MultiKrumAggregator(UserAggregator):
    def krum(self, user_grad, user_num):
        ...
        with torch.no_grad():
            ...
            u_score, select_u = torch.topk(sm_user_scores, k=self.args.multi_krum_num, largest=False)
            
        agg_grad = {}
        for name in user_grad:
            agg_grad[name] = torch.mean(user_grad[name][select_u], dim=0)
        return agg_grad
